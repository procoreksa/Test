...
import {
  ...
  activateTenantPortalAccount,
  suspendTenantPortalAccount,
  disableTenantPortalAccount,
  resetTenantPortalAccountPassword,
} from "@/lib/actions/tenant-portal-account";
...
/** The transitions the internal `transitionAccount()` helper actually permits, kept in sync so this panel never renders a button whose click would just throw `invalidAccountTransition`. */
...
/** Step 79/81 - the one-time plaintext temporary password never touches the URL, a cookie, or storage: it lives only in this client component's in-memory state for the current render and is discarded the moment the admin closes the reveal box or navigates away. */
export function TenantPortalAccountPanel({ renterId, defaultEmail, account, perms, t }: { renterId: string; defaultEmail: string; account: Account | null; perms: { canView: boolean; canCreate: boolean; canActivate: boolean; canSuspend: boolean; canDisable: boolean; canResetPassword: boolean }; t: Labels }) {
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {perms.canActivate && ALLOWED_TRANSITIONS[account.status].includes("ACTIVE") && (
              <form action={activateTenantPortalAccount}>
            ...
            {perms.canSuspend && ALLOWED_TRANSITIONS[account.status].includes("SUSPENDED") && (
              <form action={suspendTenantPortalAccount}>
                <input type="hidden" name="accountId" value={account.id} />
            ...
            {perms.canDisable && ALLOWED_TRANSITIONS[account.status].includes("DISABLED") && (
              <form action={disableTenantPortalAccount}>
                <input type="hidden" name="accountId" value={account.id} />
