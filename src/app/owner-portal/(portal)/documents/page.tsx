import { getOwnerDocuments } from "@/lib/actions/owner-portal/documents";
...
export default async function OwnerDocumentsPage() {
  const [documents, locale] = await Promise.all([getOwnerDocuments(), getLocale()]);
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm divide-y divide-slate-100">
        {documents.map((doc) => (
          <div key={doc.documentId} className="p-4 flex items-center justify-between gap-4">
            ...
            {doc.canDownload ? (
              <a
                href={`/api/owner-portal/documents/${doc.documentId}/download`}
                className="bg-brand-gold hover:bg-brand-gold-dark text-brand-black rounded-lg px-4 py-2 font-semibold text-sm whitespace-nowrap"
              >
