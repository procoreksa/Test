...
export default async function OwnerPortalLoginPage({
  searchParams,
}: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-brand-black px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 border border-brand-gold/20">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 tracking-wide">{t.ownerPortal.loginTitle}</h1>
          <p className="text-slate-500 text-sm mt-3">{t.ownerPortal.loginSubtitle}</p>
        </div>

        {error && <div className="mb-4 rounded-lg bg-red-50 text-red-700 text-sm px-4 py-3 border border-red-200">{t.ownerPortal.loginError}</div>}

        <form action={ownerLoginAction} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{t.ownerPortal.loginEmail}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{t.ownerPortal.loginPassword}</label>
