import { getMoveOutCompletionReport } from "@/lib/actions/move-out-reports";
import { getLocale, getDictionary, shortDateFormatter, pickLocalized } from "@/lib/i18n";
import { PrintButton } from "@/components/print-button";

export default async function MoveOutCompletionReportPage() {
  const [rows, locale] = await Promise.all([getMoveOutCompletionReport(), getLocale()]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between no-print">
        <h1 className="text-2xl font-bold text-slate-900">{t.operations.reportMoveOutCompletion}</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-slate-500 text-right">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">{t.moveOut.fieldStartedAt}</th>
              <th className="px-4 py-3 font-medium">{t.moveOut.fieldCompletedAt}</th>
              <th className="px-4 py-3 font-medium">{t.moveOut.reportDurationDaysLabel}</th>
            </tr>
          </thead>
