import { getMoveOutUnitConditionReport } from "@/lib/actions/move-out-reports";
import { getLocale, getDictionary, pickLocalized } from "@/lib/i18n";
import { PrintButton } from "@/components/print-button";

export default async function MoveOutUnitConditionReportPage() {
  const [rows, locale] = await Promise.all([getMoveOutUnitConditionReport(), getLocale()]);
  const t = getDictionary(locale);

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between no-print">
        <h1 className="text-2xl font-bold text-slate-900">{t.operations.reportMoveOutUnitCondition}</h1>
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((item) => (
              <tr key={item.id}>
                ...
                <td className="px-4 py-3 text-slate-500">{t.inspectionCategory[item.category]}</td>
                <td className="px-4 py-3 text-slate-700">{pickLocalized(locale, item.itemNameAr, item.itemName)}</td>
                <td className="px-4 py-3 text-slate-500">{item.moveInInspectionItem?.condition ? t.conditionRating[item.moveInInspectionItem.condition] : "—"}</td>
