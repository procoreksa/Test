...
import { getOccupantAllocationReport } from "@/lib/actions/corporate-housing-reports";
...
export default async function OccupantAllocationReportPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const { page } = await searchParams;
  const [{ rows, page: p, totalPages }, locale] = await Promise.all([getOccupantAllocationReport(page ? Number(page) : 1), getLocale()]);
  ...
  return (
    <div className="space-y-6">
            ...
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td className="px-4 py-3 text-slate-500">
                  {r.unit.unitNumber} — {unitLocationLabel(locale, r.unit)}
                </td>
                ...
                <td className="px-4 py-3 text-slate-500">{t.corporateHousingAllocationStatus[r.status]}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
      ...
      <ReportPager
        ...
        hrefFor={(n) => `/corporate-housing/reports/occupant-allocation?page=${n}`}
