...
import { getPlannedArrivalsReport } from "@/lib/actions/corporate-housing-reports";
...
export default async function PlannedArrivalsReportPage({ searchParams }: { searchParams: Promise<{ window?: string; page?: string }> }) {
  const { window, page } = await searchParams;
  const windowDays = window === "7" ? 7 : 30;
  const [{ rows, page: p, totalPages }, locale] = await Promise.all([getPlannedArrivalsReport(windowDays, page ? Number(page) : 1), getLocale()]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center justify-between mt-2">
          <h1 className="text-2xl font-bold text-slate-900">{t.corporateHousing.reportPlannedArrivals}</h1>
          <div className="flex gap-2 text-sm">
            <Link href="/corporate-housing/reports/planned-arrivals?window=7" className={`px-3 py-1.5 rounded-lg ${windowDays === 7 ? "bg-brand-gold text-brand-black font-semibold" : "bg-slate-100 text-slate-600"}`}>
              7d
            </Link>
            <Link href="/corporate-housing/reports/planned-arrivals?window=30" className={`px-3 py-1.5 rounded-lg ${windowDays === 30 ? "bg-brand-gold text-brand-black font-semibold" : "bg-slate-100 text-slate-600"}`}>
              30d
            </Link>
          </div>
        </div>
      ...
      <ReportPager
        ...
        hrefFor={(n) => `/corporate-housing/reports/planned-arrivals?window=${windowDays}&page=${n}`}
