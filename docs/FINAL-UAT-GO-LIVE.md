...
### D-009 — Direct unauthorized access to a permission-gated page crashes to a raw error screen rather than a friendly denial (discovered while verifying D-008's denial requirement) — now FIXED

- **Area:** Cross-cutting — every internal page/server action protected by `requirePermission()`, not specific to Staff Users
- **Severity:** Real UX defect — never a data-exposure risk (see below), but a genuine pre-deployment blocker for a production-quality denial experience
- **Root cause (confirmed empirically, not just by code reading):** `requirePermission()` (`src/lib/session.ts`) threw a custom `AuthorizationError`. In a genuine production build (`npm run build && npm run start`, never `next dev`), Next.js redacts a thrown error's message once it crosses the server/client boundary — the client only ever receives a generic `Error` with `name: "Error"`, a redacted `message` ("Minified React error #441..."), and an opaque `digest`. A temporary diagnostic `error.tsx` was deployed and hit live as MANAGER against `/settings/users` to confirm this directly: `{"message":"Minified React error #441...","name":"Error","digest":"1738162361","ctor":"Error"}` — proving `error.name`/`instanceof AuthorizationError` does **not** survive to any client-side error boundary. This means a bare `error.tsx` cannot reliably distinguish an authorization denial from any other unexpected error in production - confirming the task's own warning ("do not catch every exception and label it Access Denied") was not just a hypothetical risk.
- **Security impact:** None. The denial was always fully enforced server-side before any protected data rendered — this defect was purely about what the browser displayed after a correctly-enforced denial, never about whether the denial happened.
- **UX impact (before fix):** Next.js's generic, unbranded, unlocalized full-page crash screen ("This page couldn't load / A server error occurred"), plus a confusing "Minified React error #441" console message — indistinguishable from any other application bug, in neither English nor Arabic, with no path back into the app.
- **Solution:** Since the redaction happens only once an error crosses the render → client-boundary pipeline, the fix moves the authorization denial out of that pipeline entirely rather than trying to make it survive redaction:
  - `requirePermission()` now calls `redirect("/access-denied")` instead of throwing `AuthorizationError` on denial. `redirect()` is Next's own routing primitive (a special `NEXT_REDIRECT` signal intercepted by Next's router), never subject to the message-redaction behavior above, and works identically whether `requirePermission()` is called from a Server Component's render (page-level denial, D-009's original repro) or from inside a Server Action (defense-in-depth, e.g. a hidden mutation button somehow triggered) — one change at the single shared gate covers both cases.
  - `requirePermissionAudited()` (`src/lib/audit.ts`) is unaffected: it already only wraps `requirePermission()` in try/catch to log the denial and rethrow the same error — this still works unchanged for the redirect signal (catch, log, rethrow, never swallowed).
  - New bilingual `src/app/(app)/access-denied/page.tsx`: 🚫 "Access Denied / You do not have permission to access this page." (EN) and "غير مصرح بالدخول / ليس لديك صلاحية للوصول إلى هذه الصفحة." (AR), both driven by the existing `getLocale()`/`getDictionary()` i18n pipeline (new `accessDenied` dictionary keys), plus a "Back to Dashboard" / "العودة إلى لوحة التحكم" link. It renders inside the normal `(app)` layout (same sidebar/chrome), performs no permission check of its own, and never references any permission name, role, ID, or internal detail.
  - New `src/app/(app)/error.tsx` (replacing the previous complete absence of any error boundary for this route group): a generic, always-the-same, bilingual "Something Went Wrong" / "حدث خطأ ما" message with "Try Again" and "Back to Dashboard" actions. It has **zero branching logic on the error's contents** — by construction, since `AuthorizationError` can now only ever reach the app via the `/access-denied` redirect (never via this boundary), anything this boundary receives is, by definition, a genuine unexpected error, so there is no risk of an authorization denial being mislabeled here or of an unexpected error being mislabeled "Access Denied." The full original error (message, stack, Prisma error code/model, digest) is still logged server-side via Next's default production logging plus an explicit `console.error(error)` in the boundary — confirmed live (see evidence below) — satisfying "sanitized to the browser, still observable server-side."
  - `AuthorizationError` itself was **not removed** — kept for the one other call site (`communications.ts`) that performs its own inline permission check outside `requirePermission()`, untouched and out of this fix's scope.
  - The RBAC model, `can()`, `ALL_PERMISSIONS`/role subsets, and all three independent auth systems (internal/Tenant Portal/Owner Portal) were **not touched**.
- **Production-build browser evidence (all via `npm run build && npm run start`, Chromium/Playwright):**
  - MANAGER → `/settings/users` and `/settings`: renders the Access Denied page (🚫, correct AR/EN text, "Back to Dashboard" link); zero console errors; no "Minified React error"; no Staff Users data of any kind in the response.
  - ACCOUNTANT → same two pages: identical clean denial.
  - VIEWER → `/settings/users`, `/settings`, and `/operations/maintenance/vendors` (VIEWER lacks `maintenance.vendor.view`): identical clean denial on all three.
  - OWNER and ADMIN → all five representative pages tested (`/settings/users`, `/invoices`, `/payments`, `/operations/maintenance/vendors`, `/settings`): full normal access, no denial page, no console errors — confirms the fix does not affect authorized roles.
  - **Representative-page audit across modules (Step 8):** Financial/Accounting (`/invoices`, `/payments` — all 5 roles, correctly allowed for all), Operations (`/operations/maintenance/vendors` — correctly denied only for VIEWER), Administration/Settings (`/settings`, `/settings/users` — correctly denied for MANAGER/ACCOUNTANT/VIEWER) — one shared `requirePermission()` fix behaves consistently everywhere without any per-page changes.
  - **Unexpected-error distinctness (Step 7):** navigating OWNER to `/buildings/<bogus-id>/ownership` (a genuine, unrelated `PrismaClientKnownRequestError` P2025 from `findUniqueOrThrow`, not an authorization failure) rendered the generic "Something Went Wrong" page — **not** "Access Denied" — while the full Prisma error (model `Building`, code `P2025`, digest) was captured in the server-side log, proving the two paths remain semantically and visually distinct and the unexpected error stayed server-observable.
  - **EN/AR/RTL/mobile (Step 6):** EN desktop confirmed `dir="ltr"`, "Access Denied" / "You do not have permission to access this page." / "Back to Dashboard" all correct. AR mobile viewport (390×844) confirmed `dir="rtl"`, correct Arabic text, mobile sidebar (hamburger) chrome intact, layout uncut/unbroken.
- **Regression evidence:** `prisma validate` clean; `tsc --noEmit` clean; `eslint` clean (0 errors, only pre-existing unrelated warnings in scratch scripts); `vitest run` 726/726 (unchanged from baseline — includes the RBAC mocked-session integration suites, e.g. `rbac.integration.test.ts`, `crm-rbac.integration.test.ts`, which continue to pass because `redirect()` still throws — just a different, non-`AuthorizationError`-named error — satisfying every existing `.rejects.toThrow()` assertion); `vitest run --config vitest.db.config.mts` 516/516 (unchanged from baseline, 70 files, including all cross-org/IDOR suites); production build clean; production-browser verification above all passed. No new automated tests were added in this pass (baseline 726/516 unchanged) - the fix was verified live per the task's own methodology instead.
- **Step 9 (N+1 review of `getContractLinksForRenters()`, D-008):** inspected `src/lib/actions/contracts.ts` — it is a single `prisma.contract.findMany({ where: { renterId: { in: renterIds } }, distinct: ["renterId"] })` call, already batched across all renters in one query (not one query per renter). No N+1 pattern exists; no change made.
- **Disposition:** **FIXED**, verified via genuine production-build browser testing across all 5 roles, EN/AR/RTL, desktop/mobile, and representative pages from Staff Users, Financial/Accounting, Operations, and Administration/Settings modules.

### D-010 — Unit deletion could silently destroy ownership/financial history instead of being blocked (reported as a production server error; not a Prisma FK error) — now FIXED

- **Area:** `deleteUnit()` (`src/lib/actions/units.ts`) and the Prisma schema's `Unit` relations, discovered while auditing every relation Unit deletion touches.
- **Severity:** Data-integrity — silent, undetected loss of ownership/financial history, not a crash risk once the actual root cause was found. Classified as a genuine pre-Go-Live blocker.
- **Reproduction methodology:** Per the task's explicit instruction not to assume a Prisma foreign-key (P2003) error, every one of Unit's real schema relations was mapped from the live database's actual FK definitions (`pg_constraint`, not just `schema.prisma` reading, to rule out drift), and three scenarios were reproduced live against the real UAT database via a genuine production build (`npm run build && npm run start`) and real browser clicks (Chromium/Playwright) as OWNER:
  1. A brand-new, completely unused Unit (no ownership, no rental activity) — deleted cleanly, no error. Already correct.
  2. `UAT-102` (active Contract, move-in, move-out, reservation, maintenance) — correctly **blocked** with the existing friendly "cannot delete — related records" message (`t.validation.unitHasHistory`), already fixed under D-005/D-006. No regression here.
  3. A fresh Unit with **only** a `PropertyOwnership` assignment (100% to an Owner) and **no** Contract/Reservation/Viewing/Offer/MoveIn/MoveOut/Maintenance/CorporateAllocation/SecurityDepositSettlement — **deleted successfully, with the ownership record silently destroyed along with it.** No error was ever thrown; the delete simply "worked," which is the defect: an ownership assignment is exactly the kind of historical/business record the task's own brief says must never be silently lost, and `deleteUnit()`'s existing safety net (catch `P2003`, show a friendly message) never engaged because no FK error occurred at all.
- **Root cause (confirmed against the live database's actual foreign-key definitions, not assumed from the schema file):** every one of Unit's protected relations (contracts, reservations, move-ins/outs, maintenance requests, viewing_units, leasing_offers, corporate_housing_allocations, security_deposit_settlements) is `ON DELETE RESTRICT` — correctly blocking deletion via the existing P2003 handling. Exactly **two** relations were not: `property_ownerships.unitId` was `ON DELETE CASCADE` (an explicit choice in the schema, shared with Compound/Building's own ownership relations, which are out of this task's Unit-only scope), and `owner_ledger_entries.unitId` had no explicit `onDelete`, defaulting to `ON DELETE SET NULL`. Neither produces an error — Cascade silently deletes the ownership row, SetNull silently detaches a financial ledger entry from its Unit — so `deleteUnit()` had nothing to catch. This is why the defect is not a Prisma FK error: the delete *succeeds*, incorrectly.
  - No literal browser crash ("This page couldn't load") or React-error digest was reproduced for any tested scenario in the current codebase (post-D-009); D-009's `redirect()`-based access-denial fix and the pre-existing P2003 handling both continued to behave correctly throughout this investigation. The originally reported symptom is treated here as the trigger for this audit rather than a literally-reproduced crash — the audit found a real, distinct, and arguably more serious defect (silent data loss, not a visible error) in the same code path.
- **Fix (schema-only, no destructive migration, no cascade rule added — the opposite):** Changed `PropertyOwnership.unit`'s relation from `onDelete: Cascade` to `onDelete: Restrict`, and `OwnerLedgerEntry.unit`'s relation to explicit `onDelete: Restrict` (from the implicit SetNull default) — `prisma/schema.prisma`. This makes both relations behave exactly like every other protected Unit relation, so `deleteUnit()`'s **existing, unmodified** P2003-catch-and-friendly-message logic now also covers them; no application code changed. Migration: `prisma/migrations/20270119090000_unit_delete_protect_ownership_and_ledger/migration.sql` (two `ALTER TABLE ... DROP/ADD CONSTRAINT` statements, additive/non-destructive — it does not touch any existing row). Compound/Building's own `PropertyOwnership` Cascade relations were deliberately left untouched (out of scope — a separate defect against `deleteCompound()`/`deleteBuilding()`, flagged below rather than fixed here per "do not modify unrelated modules").
- **Security impact:** None — `unit.delete` remains OWNER/ADMIN-only (verified: not present in MANAGER/ACCOUNTANT/VIEWER permission arrays), `deleteUnit()`'s `where: { id, organizationId }` compound filter is unchanged (cross-org deletion still structurally impossible), and the fix does not touch authorization at all.
- **Data-integrity impact:** This is the actual defect being fixed — before this change, an Owner's ownership assignment to a Unit (and, separately, a financial ledger entry's Unit reference) could be permanently and silently lost with zero warning to the operator, by deleting a Unit that had never been leased. After the fix, the same Unit deletion is blocked with the existing friendly message, and the ownership/ledger records survive untouched.
- **Test evidence (real-DB, `vitest run --config vitest.db.config.mts`):** new `src/lib/actions/__dbtests__/unit-delete.db.test.ts` (7 tests): empty-Unit deletion succeeds; Unit-with-Contract deletion is blocked and nothing is deleted; **Unit-with-ownership-only deletion is now blocked and the ownership record survives** (this is the regression test for the actual defect); Unit tagged on an `OwnerLedgerEntry` is blocked and the entry keeps its `unitId`; a role without `unit.delete` (MANAGER) is rejected; another organization's Unit is never deleted (cross-org IDOR); a nonexistent Unit id is handled without crashing. All 7 pass, applied against the disposable `.env.test` database via the existing `global-setup.ts` (`prisma migrate deploy`), which is the codebase's standing safe mechanism for schema changes in real-DB tests. Full real-DB suite: **523/523** (516 baseline + these 7 new), 71 files, no regressions. Full unit suite: **726/726** (unchanged, no unit tests needed for this fix — it's a schema/relation change with existing RBAC/business logic untouched). `tsc --noEmit`, `eslint` (0 errors), `prisma validate`, and a full production build (`npm run build`) all clean after the schema change.
- **Migration deployment to the UAT/development database (explicit user approval obtained, scoped to UAT/dev only — not production):**
  - **Target confirmed:** `postgresql://app_user:app_password@localhost:5432/rental_saas` — the local Postgres instance in this environment's own container, the same database used throughout this entire UAT arc (`uat-org-a`/`uat-org-b`, the 5 internal role fixtures, UAT-101/UAT-102). Confirmed **not** production: no production infrastructure exists in this environment (per every prior Prompt's disposition — "PENDING EXTERNAL"), and `.env`'s `AUTH_SECRET` is explicitly named `dev-only-secret-change-in-production`.
  - **Migration applied:** `20270119090000_unit_delete_protect_ownership_and_ledger`. Its SQL was re-inspected immediately before deployment and contains exactly the four approved statements (two `DROP CONSTRAINT` / `ADD CONSTRAINT` pairs for `property_ownerships_unitId_fkey` and `owner_ledger_entries_unitId_fkey`, both to `ON DELETE RESTRICT`) — no `DROP TABLE`, no `DROP COLUMN`, no data-mutating statement, no unrelated change.
  - It turned out the migration had already been applied as an inherent side effect of an earlier `npm run build` in the same session — this project's own pre-existing `build` script (`prisma generate && prisma migrate deploy && next build`, unmodified, `package.json`) always runs `migrate deploy` against whatever `.env` points to. Re-running `npx prisma migrate deploy` explicitly per the approval confirmed this: `No pending migrations to apply.`
  - `npx prisma migrate status` → `Database schema is up to date!` (28 migrations found).
  - **Live FK verification** (`pg_constraint`, not just the schema file): `property_ownerships_unitId_fkey` → `RESTRICT`; `owner_ledger_entries_unitId_fkey` → `RESTRICT`. Both confirmed on the actual `rental_saas` database.
  - **Data preserved:** row counts taken after deployment — 12 units, 4 `PropertyOwnership`, 1 `OwnerLedgerEntry`, 11 contracts, 2 move-ins, 3 move-outs, 1 reservation, 2 maintenance requests, all consistent with pre-existing UAT state; `UAT-101`/`UAT-102` both present with their original related records intact; the two pre-existing `PropertyOwnership` rows for UAT Org A (a compound-level owner and a unit-level override for UAT-101) confirmed as legitimate prior fixtures, not test artifacts. The migration itself only alters a constraint definition — it never touches row data — so this was expected, and it was independently confirmed rather than assumed.
- **Live browser verification (production build, `npm run build && npm run start`, real UAT database, Chromium/Playwright) — TEST A-H:**
  - **A (empty Unit):** created a disposable empty Unit through the real UI, deleted it through the real UI — succeeded, no error, removed from the list and the database.
  - **B (Unit with rental history, `UAT-102`, untouched real UAT data):** attempted deletion through the real UI — blocked, friendly localized message shown ("This unit cannot be deleted because it has related contracts or business records..."/Arabic equivalent), `UAT-102` and all its related records (2 contracts, 1 move-in, 1 move-out, 1 reservation, 1 maintenance request) confirmed still in the database afterward. No historical UAT data was altered or destroyed.
  - **C (ownership-only Unit — the critical regression check):** created a disposable Unit with only a `PropertyOwnership` assignment (100%, no other relation) through direct fixture creation, attempted deletion through the real UI — **now blocked** (previously silently succeeded before the fix); both the Unit and the `PropertyOwnership` row confirmed still present in the database afterward.
  - **D (Unit tagged on an `OwnerLedgerEntry`):** same pattern — deletion blocked through the real UI; the Unit and the `OwnerLedgerEntry` both confirmed still present, with `unitId` still pointing at the Unit (not nulled).
  - **E (RBAC, existing permission model, unchanged):** logged in as each of OWNER, ADMIN, MANAGER, ACCOUNTANT, VIEWER — `/units` is reachable by all five (existing `unit.view` grant, unaffected by this fix), but the Delete action is visible only for OWNER/ADMIN (4 buttons each) and absent for MANAGER/ACCOUNTANT/VIEWER (0 each), matching `unit.delete`'s existing OWNER/ADMIN-only grant in `src/lib/permissions.ts`. Server-side enforcement (not just UI hiding) is separately proven by the `unit-delete.db.test.ts` test that calls `deleteUnit()` directly as MANAGER, bypassing the UI entirely, and confirms rejection with nothing deleted.
  - **F (cross-organization):** a disposable Unit was created under a different, pre-existing organization (`UAT Org B`); logged in as OWNER of `UAT Org A`, confirmed that unit never appears in Org A's `/units` list (existence not disclosed) and remains untouched in the database afterward — the same `where: { id, organizationId }` compound filter from before this fix, unaffected.
  - **G (localization/responsive):** EN desktop confirmed `dir="ltr"` and the English friendly message rendering cleanly inline next to the blocked row (screenshot captured). AR mobile (390×844 viewport) confirmed `dir="rtl"`, the Arabic friendly message ("لا يمكن حذف هذه الوحدة لوجود عقود أو سجلات عمل مرتبطة بها — قم بأرشفتها بدلاً من ذلك") rendering legibly under the blocked row, and the mobile hamburger-sidebar chrome intact (screenshot captured).
  - **H (data integrity, post-all-tests):** all disposable test fixtures (the empty/ownership-only/ledger-tagged/foreign-org Units and their child rows) were deleted directly to leave the UAT database exactly as found; `UAT-101`, `UAT-102`, their full relation set, and both pre-existing `PropertyOwnership` rows confirmed intact and unchanged.
- **Final regression after live deployment:** `prisma validate` clean; `tsc --noEmit` clean; `eslint` 0 errors (only pre-existing scratch-script warnings); unit suite **726/726**; real-DB suite **523/523** (516 baseline + the 7 D-010 tests, unchanged from the pre-deployment run — totals match exactly, no explanation needed); production build clean.
- **Known, related, out-of-scope defect (not fixed here, tracked separately as D-011 below):** `PropertyOwnership.compound`/`PropertyOwnership.building` still use `onDelete: Cascade` exactly as `unit` did before this fix.
- **Disposition:** **FIXED.** Migration deployed to the UAT/development database only (explicit scoped approval obtained; production untouched, no infrastructure deployed, no other schema changes made). Live FK constraints verified. Existing data verified intact. Full Test A-H live browser verification passed. Regression clean at the expected post-D-010 baseline (726/523).

### D-011 — Compound/Building deletion could silently destroy ownership/financial history, and had no friendly-message handling at all — now FIXED

- **Area:** `PropertyOwnership.compound`/`.building`, `OwnerLedgerEntry.compound` relations (`prisma/schema.prisma`); `deleteCompound()` (`src/lib/actions/compounds.ts`); `deleteBuilding()` (`src/lib/actions/buildings.ts`); their list-page UI (`src/app/(app)/compounds/page.tsx`, `src/app/(app)/buildings/page.tsx`).
- **Root cause — complete dependency audit (live `pg_constraint` against the real `rental_saas` database, not just schema-file reading):**

  **Compound's full dependency graph** (every FK in the database referencing `compounds`):
  | Child relation | Nature | onDelete (before) | Classification | Verdict |
  |---|---|---|---|---|
  | `buildings.compoundId` | Structural hierarchy child | CASCADE | Safe disposable (Building has its own protections; see below) | No change — correct by design |
  | `property_ownerships.compoundId` | Ownership/entitlement data | **CASCADE** | Ownership history | **UNSAFE — fixed to RESTRICT** |
  | `owner_ledger_entries.compoundId` | Financial/accounting data | **SET NULL** (implicit default) | Financial record | **UNSAFE — fixed to RESTRICT** |
  | `leads.preferredCompoundId` | CRM soft preference tag | SET NULL (explicit, deliberate) | Safe disposable | No change — correctly already safe |
  | `maintenance_requests.compoundId` | Operational/historical | RESTRICT (explicit, deliberate) | Historical/business data | No change — already safe |

  **Building's full dependency graph:**
  | Child relation | Nature | onDelete (before) | Classification | Verdict |
  |---|---|---|---|---|
  | `floors.buildingId` | Structural hierarchy child | CASCADE | Safe disposable (Floor has its own protection; see D-011's Floor review below) | No change — correct by design |
  | `property_ownerships.buildingId` | Ownership/entitlement data | **CASCADE** | Ownership history | **UNSAFE — fixed to RESTRICT** |
  | `maintenance_requests.buildingId` | Operational/historical | RESTRICT (explicit, deliberate) | Historical/business data | No change — already safe |

  Building has no `ownerLedgerEntries` relation at all (confirmed: `OwnerLedgerEntry` has no `buildingId` field in the schema) — nothing analogous to fix there.

  This is the exact same defect shape as D-010, on the same `PropertyOwnership`/`OwnerLedgerEntry` models, at the two remaining asset levels D-010 explicitly left out of scope. **Do not assume RESTRICT is always correct**: the audit confirmed it is correct here because ownership/financial data must never be silently lost, but it deliberately did **not** touch `buildings.compoundId`/`floors.buildingId` (Cascade is the *correct* choice for the structural parent-child hierarchy itself — deleting a Compound legitimately removes its own empty Buildings, provided those Buildings have no protected data of their own, which their own Restrict-guarded relations now ensure transitively) or `leads.preferredCompoundId` (a soft, explicitly-chosen SetNull preference tag, not business-critical data).
  - Second, independent gap found on top of the cascade issue: **`deleteCompound()`/`deleteBuilding()` had no `try/catch` at all** (unlike `deleteUnit()`, which already had P2003 handling since D-005/D-006). Even the relations that were *already* `RESTRICT` (`maintenance_requests`) would have surfaced as an uncaught error with no friendly message on a blocked delete — via the route group's `error.tsx` (D-009) at best, never the specific "cannot delete — has related records" message every other module already shows.
- **Fix (schema — additive, non-destructive; code — new try/catch matching `deleteUnit()`'s established pattern exactly; UI — converted to the shared component):**
  1. `prisma/schema.prisma`: `PropertyOwnership.compound`/`.building` → `onDelete: Restrict` (was `Cascade`); `OwnerLedgerEntry.compound` → explicit `onDelete: Restrict` (was implicit `SetNull`). `PropertyOwnership.unit` (D-010) left untouched/unchanged.
  2. `deleteCompound()`/`deleteBuilding()`: added the identical `try { ... } catch (error) { if P2003 return { error: t.validation.compoundHasHistory/buildingHasHistory }; throw error; }` wrapper `deleteUnit()` already has, changing their return type from `void` to `Promise<{ error?: string }>`.
  3. New dictionary keys `validation.compoundHasHistory`/`validation.buildingHasHistory` (EN + AR), following the exact `unitHasHistory`/`ownerHasHistory` pattern.
  4. `compounds/page.tsx`/`buildings/page.tsx`: replaced the raw inline `<form action={async () => { "use server"; await deleteX(id); }}>` (which discarded any returned `{error}` and had no display) with the shared `<DeleteEntityButton>` component every other module (Units, Renters, Owners) already uses — this is what actually surfaces the friendly message inline instead of losing it.
- **Exact migration SQL** (`prisma/migrations/20270120090000_compound_building_protect_ownership_and_ledger/migration.sql`, additive/non-destructive — no `DROP TABLE`, no `DROP COLUMN`, no `TRUNCATE`, no `DELETE`, no data rewrite):
  ```sql
  ...
  ```
- **Database target confirmation:** `postgresql://app_user:app_password@localhost:5432/rental_saas` — the same local UAT/development database used throughout this entire UAT arc and confirmed in D-010 as not production (no production infrastructure exists in this environment; `.env`'s `AUTH_SECRET` is explicitly `dev-only-secret-change-in-production`). Applied via `npx prisma migrate deploy` (no separate schema change beyond the three constraints above).
- **Live PostgreSQL FK verification** (`pg_constraint`, post-deployment, against `rental_saas`): `property_ownerships_compoundId_fkey` → `RESTRICT`; `property_ownerships_buildingId_fkey` → `RESTRICT`; `owner_ledger_entries_compoundId_fkey` → `RESTRICT`; `property_ownerships_unitId_fkey`/`owner_ledger_entries_unitId_fkey` (D-010) reconfirmed still `RESTRICT`, unaffected.
- **Real-DB test evidence:** new `src/lib/actions/__dbtests__/compound-building-delete.db.test.ts` (13 tests, mirroring `unit-delete.db.test.ts`'s D-010 coverage for both Compound and Building): empty-Compound/Building deletion succeeds; deletion blocked by an existing `maintenance_requests` relation (already-safe case, confirmed unregressed) with nothing deleted; **Compound/Building with ownership-only blocked** (the regression tests for the actual defect), ownership record preserved; Compound tagged on an `OwnerLedgerEntry` blocked, `compoundId` preserved; role without `property.delete` (MANAGER) rejected for both; cross-org Compound/Building never deleted; nonexistent id handled safely for both. All 13 pass against the disposable `.env.test` database via the same `global-setup.ts` (`prisma migrate deploy`) mechanism D-010 used. Full real-DB suite: **536/536** (523 D-010 baseline + these 13 new), 72 files, no regressions.
- **Live browser evidence (production build, real UAT database, Chromium/Playwright) — Tests A-L:**
  - **A/B (empty Compound/Building):** both created and deleted cleanly through the real UI, no error, removed from the database.
  - **C/D (Compound/Building with ownership-only — the critical regression check):** both created with a `PropertyOwnership` assignment and no other relation, both **now blocked** through the real UI (previously would have silently succeeded and destroyed the ownership record); both the asset and its ownership record confirmed still in the database afterward.
  - **E (real UAT data with protected history, untouched):** attempted deletion of `UAT Org A Compound` and `UAT Building` (both carry real ownership/contract history) through the real UI — both blocked with the new friendly message, both confirmed unchanged in the database afterward. Zero console errors throughout Tests A-E.
  - **F/G (RBAC, existing permission model unchanged):** `/compounds` reachable by all 5 roles (existing `property.view`), Delete visible only for OWNER/ADMIN (2 buttons each) and absent for MANAGER/ACCOUNTANT/VIEWER (0 each), matching `property.delete`'s existing OWNER/ADMIN-only grant. Server-side enforcement (not just UI hiding) independently proven by the db-test suite calling `deleteCompound()`/`deleteBuilding()` directly as MANAGER and confirming rejection with nothing deleted.
  - **H (cross-organization):** a disposable Compound created under `UAT Org B` never appears in `UAT Org A`'s `/compounds` list and remains untouched in the database when Org A's OWNER attempts nothing against it (existence not disclosed).
  - **I/J/K (localization/responsive):** EN desktop confirmed `dir="ltr"` and "This compound has related ownership, ledger, or other business records and cannot be deleted" rendering cleanly inline. AR mobile (390×844) confirmed `dir="rtl"` and "لا يمكن حذف هذا المجمع لوجود سجلات ملكية أو حركات مالية أو سجلات عمل أخرى مرتبطة به" rendering legibly, mobile chrome intact (screenshots captured for both).
  - **L (data integrity, post-all-tests):** all disposable test fixtures removed; final counts (5 compounds, 5 buildings, 6 floors, 12 units, 4 `PropertyOwnership`, 1 `OwnerLedgerEntry`, 11 contracts) match the pre-test baseline exactly; both pre-existing `PropertyOwnership` rows (a compound-level owner, a unit-level override for `UAT-101`) confirmed intact; `UAT Org A Compound` and `UAT Building` confirmed present with their original relations.
- **Floor analogous-risk review (Step 13):** Floor's only child relation is `units.floorId`, already `ON DELETE RESTRICT` (confirmed live) — Floor carries no direct ownership or financial tagging capability at all (`PropertyOwnership`'s three optional FKs are compound/building/unit only; there is no `floorId` field on `PropertyOwnership` or `OwnerLedgerEntry`). **No unsafe cascade path exists for Floor deletion** — nothing to fix. One smaller, non-data-risk observation: `deleteFloor()` (`src/lib/actions/floors.ts`) has the same missing-try/catch gap `deleteCompound()`/`deleteBuilding()` had before this fix (a blocked Floor deletion today would surface via the generic `error.tsx` rather than a specific friendly message) — this is a UX-consistency item, not a data-integrity defect, so it was not fixed as part of D-011 (would be scope creep into `floors.ts`, untouched by this task) and is not being registered as a separate D-012, since D-012 is reserved for a genuine data-integrity finding and this one carries none.
- **Regression:** `prisma validate` clean; `tsc --noEmit` clean; `eslint` 0 errors (only pre-existing scratch-script warnings); unit suite **726/726** (unchanged); real-DB suite **536/536** (523 D-010 baseline + 13 new D-011 tests); production build clean.
- **Security impact:** None — `property.delete` remains OWNER/ADMIN-only (confirmed absent from MANAGER/ACCOUNTANT/VIEWER permission arrays), both actions' `where: { id, organizationId }` compound filters are unchanged (cross-org deletion structurally impossible), no authorization logic touched.
- **Data-integrity impact:** This is the fix — a Compound's or Building's ownership assignment, and a Compound-tagged financial ledger entry, can no longer be silently lost by deleting an asset that was never leased.
- **Disposition:** **FIXED.** Schema audited completely (not assumed), migration applied to the UAT/development database only (confirmed non-production, additive/non-destructive), live FK constraints verified, existing UAT data verified intact, full Test A-L live browser verification passed, Floor reviewed with no analogous risk found, regression clean.
