...
### D-010 — Unit deletion could silently destroy ownership/financial history instead of being blocked (reported as a production server error; not a Prisma FK error)
...
- **⚠️ Known limitation — migration not yet applied to the shared UAT/dev database:** this session's tooling denied direct schema-modifying commands (`prisma migrate dev`/`deploy`) against the shared development database (`rental_saas`, the one the running UAT instance and `npm run start` use) as a "modify shared resources" action requiring explicit permission. The fix was fully authored, and **proven correct** via the project's own safe, pre-existing, disposable-database real-DB test pipeline (`npm run test:db`, which runs `prisma migrate deploy` against the separate, sacrificial `.env.test` database as part of its standing `global-setup.ts` — not the shared UAT database), but the actual UAT/dev database's live schema **still has the old `Cascade`/`SetNull` behavior** until someone with permission runs `npx prisma migrate deploy` (or `migrate dev`) against it. **Live interactive browser re-verification of this specific fix (Step 10 of the brief — clicking through as each of the 5 roles against the real UAT data) was not possible in this pass for that reason** and should be the first thing done once the migration is applied to that database, before this defect is called fully closed end-to-end.
- **Known, related, out-of-scope defect (not fixed here):** `PropertyOwnership.compound`/`PropertyOwnership.building` still use `onDelete: Cascade` exactly as `unit` did before this fix — `deleteCompound()`/`deleteBuilding()` likely have the identical silent-ownership-loss defect for a Compound/Building with an ownership assignment but no protected child records. Out of scope for D-010 (Unit deletion only, per the task's explicit instruction not to modify unrelated modules) — recommended as a follow-up task before Go-Live, using the exact same fix pattern (Cascade → Restrict) proven safe here.
- **Disposition:** **Code-complete and verified via automated real-DB tests; NOT yet verified live against the shared UAT database** (migration not applied there — see the limitation above). Do not mark this FIXED end-to-end until the migration is applied to the UAT/dev database and Step 10's live 5-role/EN-AR/desktop-mobile browser verification is actually run against it.
