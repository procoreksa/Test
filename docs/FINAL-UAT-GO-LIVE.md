...
### D-011 — Compound/Building deletion ownership-history integrity review (documented only — NOT implemented)

- **Area:** `PropertyOwnership.compound` and `PropertyOwnership.building` relations (`prisma/schema.prisma`), and their corresponding delete actions `deleteCompound()` (`src/lib/actions/compounds.ts`) / `deleteBuilding()` (`src/lib/actions/buildings.ts`).
- **Origin:** Identified as a byproduct of the D-010 investigation, not independently investigated. **Per explicit instruction, this is documentation only — no schema change, no migration, and no code change have been made for D-011.**
- **Suspected defect (dependency analysis, not yet empirically reproduced):** Exactly the same shape as D-010's root cause. Live FK inspection during D-010 (`pg_constraint` against `rental_saas`) already confirmed `property_ownerships_compoundId_fkey` and `property_ownerships_buildingId_fkey` remain `ON DELETE CASCADE` (unchanged by the D-010 fix, which touched only the `unitId` relation). If `deleteCompound()`/`deleteBuilding()` have the same "delete then catch P2003" pattern as `deleteUnit()` (not yet confirmed by reading those two action files in this pass), then a Compound or Building with a `PropertyOwnership` assignment but no other Restrict-guarded child record would have its ownership history silently destroyed on deletion, exactly as Units did before D-010.
- **What is NOT yet done, and must happen before any remediation:** reading `deleteCompound()`/`deleteBuilding()` in full to confirm they share `deleteUnit()`'s exact pattern; mapping every FK relation Compound and Building each carry (not just `PropertyOwnership`) the same way D-010 mapped every Unit relation, since Compound/Building may have additional or different child models; confirming whether `OwnerLedgerEntry.compoundId` (already `SET NULL`, unchanged by D-010) has the same financial-record-traceability concern at the Compound level; and determining the actual current behavior empirically (does deletion silently succeed today, or does some other guard already exist) rather than assuming the Unit-level finding transfers unmodified.
- **Proposed remediation (for review, not yet approved or implemented):** the same pattern proven safe in D-010 — change `PropertyOwnership.compound`/`.building` from `onDelete: Cascade` to `onDelete: Restrict`, author an equivalent non-destructive migration, and add an equivalent real-DB test suite mirroring `unit-delete.db.test.ts` for both Compound and Building. A separate suggested task (`task_4652d6ae`) was already queued describing this exact scope.
- **Disposition:** **Documented, not implemented.** Awaiting separate review and explicit approval before any schema change is made, per instruction.
