...
| # | Issue | Affected module | Recommended future action |
|---|---|---|---|
| 1 | No login rate limiting - now **two** public, unauthenticated, abuse-sensitive endpoints exist (`/login` for internal staff, and `/portal/login` for tenants, added by the Tenant Portal). Deliberately not implemented in this pass: no in-process limiter is safe across multiple server instances without shared state (Redis/similar), which this codebase has no existing infrastructure for, and a single-process in-memory limiter would give a false sense of protection in any real multi-instance deployment while adding real complexity now. Both login paths already share the mitigations that matter most given that gap: bcrypt's own cost factor slows brute force regardless of any rate limiter, and both `authorize()` paths (`src/lib/auth.ts`, `src/lib/tenant-credentials.ts`) return an identical generic failure for every rejection reason (wrong password, nonexistent account, inactive account), so there is no username-enumeration oracle to combine with a fast-guessing attack. | `src/lib/auth.ts` / `/login`, `src/lib/tenant-credentials.ts` / `/portal/login` | Add IP- or account-based throttling once a shared store (Redis or the hosting platform's own edge rate limiting, e.g. Render's) is available - apply it to both login paths identically, not just the newer one. |
